...
```
❯ clab inspect
22:22:16 INFO Parsing & checking topology file=lab.clab.yml
╭─────────────────────┬───────────────────────────────┬─────────┬───────────────────╮
...
SR Linux version `show version`:
`Software Version     : v24.10.1`

List of interfaces `show interface brief`:
```
...
Why `ethernet-1/1` is `oper: up` whilst others are `oper: down`:
- The only namespaces that were joined in the network topology were `srl1` and `srl2` via `e1-1` bridge.
